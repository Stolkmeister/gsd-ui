...
// ---- Resolve .planning/ path ----

async function findPlanningPath(): Promise<string> {
  ...
  if (cliArg) {
    ...
    if (resolved.endsWith(".planning")) {
      await assertDirectory(resolved)
      return resolved
    }

    // Otherwise, look for .planning/ inside the given path
    const withPlanning = join(resolved, ".planning")
    try {
      await assertDirectory(withPlanning)
      return withPlanning
    } catch {
      // Fall through to try the path as-is
    }
  ...
  // Auto-detect: look for .planning/ in current working directory
  const cwdPath = join(process.cwd(), ".planning")
  try {
    await assertDirectory(cwdPath)
    return cwdPath
  } catch {
    // Not found in cwd
  }
  ...
  console.error(
    "Usage: bun run server/index.ts [path-to-project-or-.planning-directory]\n" +
      "  Or run from a directory containing a .planning/ folder."
  )
  process.exit(1)
}
...
const planningPath = await findPlanningPath()
